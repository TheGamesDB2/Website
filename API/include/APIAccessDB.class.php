...
class APIAccessDB
{
	...
	function RequestPublicAPIKey($user_id)
	{
		...
		if($sth->execute())
		{
			...
			else
			{
				...
				$key = bin2hex($bytes);
				$sth = $dbh->prepare("INSERT INTO apiusers (users_id, apikey, api_allowance_level_id, extra_allowance, is_private_key)
				VALUES(:users_id, :apikey, 1, 0, 0);");
				$sth->bindValue(':users_id', $user_id, PDO::PARAM_INT);
	...
	function RequestPrivateAPIKey($user_id)
	{
		...
		$sth = $dbh->prepare("Select apikey, extra_allowance, is_banned FROM apiusers where users_id = :user_id AND is_private_key != 0 LIMIT 1;");
